...
class VCardParserTest extends TestCase
{
    ...
    public function testFromFile()
    {
        ...
        public function testFromFileHorde()
    {
        $parser = VCardParser::parseFromFile(__DIR__ . '/horde.vcf');
        // Use this opportunity to test fetching all cards directly.
        $cards = $parser->getCards();
        
        $this->assertEquals($cards[0]->firstname, "MyFirstName");
        $this->assertEquals($cards[0]->lastname, "MyLastName");
        $this->assertEquals($cards[0]->additional, "MyMiddlename");
        $this->assertEquals($cards[0]->prefix, "MyPrefixes");
        $this->assertEquals($cards[0]->suffix, "MySuff");
        $this->assertEquals($cards[0]->fullname, "Full Name");
        $this->assertEquals($cards[0]->organization, "MyCompany;MyDepartment");
        $this->assertEquals($cards[0]->birthday->format('Y-m-d'), "2021-09-16");
        //$this->assertEquals($cards[0]->url["default"], "myUrl");
        
        // Added
        
        $this->assertEquals($cards[0]->role, "myRole");
        $this->assertEquals($cards[0]->spouse, "mySpouse");
        $this->assertEquals($cards[0]->xim, "IM1");
        $this->assertEquals($cards[0]->nickname, "MyNickname");
        $this->assertEquals($cards[0]->anniversary->format('Y-m-d'), "1912-10-15");
